    ...
    public function createResponseFromQueryBuilder(QueryBuilder $queryBuilder, FieldCollection $fields, string $filename): Response
    {
        ...
        foreach ($result as $index => $row) {
            foreach ($row as $columnKey => $columnValue) {
                ...
                if($columnValue instanceof \DateTimeInterface) $columnValue = $columnValue->format('Y-m-d H:i:s');
                if(is_array($columnValue)) $columnValue = json_encode($columnValue, JSON_THROW_ON_ERROR);                
